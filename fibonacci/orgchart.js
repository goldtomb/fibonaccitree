var OrgChart = function (t, e) {
    ...
    null != this.config.nodeMouseClickBehaviour && (this.config.nodeMouseClick = this.config.nodeMouseClickBehaviour), null != this.config.mouseScroolBehaviour && (this.config.mouseScrool = this.config.mouseScroolBehaviour), OrgChart._validateConfig(this.config) && (this.server = null, this._vScroll = {}, this.config.ui || (this.ui = OrgChart.ui), this.config.editUI ? this.editUI = this.config.editUI : this.editUI = new OrgChart.editUI, this.editUI.init(this), null === this.server && (this.server = new OrgChart.server(this.config)), this.config.searchUI ? this.searchUI = this.config.searchUI : this.searchUI = new OrgChart.searchUI, this.searchUI.init(this), this.config.nodeMenuUI ? this.nodeMenuUI = this.config.nodeMenuUI : this.nodeMenuUI = new OrgChart.menuUI, this.nodeMenuUI.init(this, this.config.nodeMenu), this.config.toolbarUI ? this.toolbarUI = this.config.toolbarUI : this.toolbarUI = new OrgChart.toolbarUI, this.config.notifierUI ? this.notifierUI = this.config.notifierUI : this.notifierUI = new OrgChart.notifierUI, this.notifierUI.init(this), this.config.dragDropMenuUI ? this.dragDropMenuUI = this.config.dragDropMenuUI : this.dragDropMenuUI = new OrgChart.menuUI, this.dragDropMenuUI.init(this, this.config.dragDropMenu), this.config.menuUI ? this.menuUI = this.config.menuUI : this.menuUI = new OrgChart.menuUI, this.menuUI.init(this, this.config.menu), this.config.xScrollUI || (this.xScrollUI = new OrgChart.xScrollUI(this.element, this.config, function () {
        ...
        r._draw(!0, OrgChart.action.xScroll)
    })), this._gragStartedId = null, this._timeout = null, this._touch = null, this._initialized = !1, this.response = null, this.nodes = null, this._event_id = OrgChart._guid(), this._setInitialSizeIfNotSet(), OrgChart.events.publish("init", [this]), 0 < this.config.nodes.length && this._draw(!1, OrgChart.action.init))
};
OrgChart.prototype.load = function (t) {
    ...
    this.server.read(t, this.width(), this.height(), e, m, O, function (t) {
        v.nodes = t.nodes, v.editUI.fields = t.allFields;
        var e = v.ui.defs(),
            r = v.getScale(t.viewBox);
        e += v.ui.pointer(v.config, m, r);
        var i = v.getViewBox(),
            a = t.viewBox;
        for (var n in v._initialized || (v.element.innerHTML = '<div id="bg-tests"></div>'), t.visibleNodes) {
            var o = t.visibleNodes[n];
            e += v.ui.node(o, v._get(n), t.animations, v.config, void 0, void 0, void 0, m, r, v), e += v.ui.secondLinks(t.visibleNodes, o, v.config, t.boundary, r), e += v.ui.link(o, v, v.config, r), e += v.ui.expandCollapse(o, v.config, m, r)
        }
        if (e += v.ui.lonely(v.config), m !== OrgChart.action.centerNode && m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update || (a = i), m === OrgChart.action.init && null != i && (a = i), m !== OrgChart.action.exporting) {
            v.response = t;
            C = v.ui.svg(v.width(), v.height(), a, v.config, e);
            if (v._initialized) {
                var l = v.getSvg(),
                    s = l.parentNode;
                s.removeChild(l), s.insertAdjacentHTML("afterbegin", C), v._attachEventHandlers(), v.xScrollUI.addListener(v.getSvg()), v.yScrollUI.addListener(v.getSvg()), v.xScrollUI.setPosition(), v.yScrollUI.setPosition()
            } else {
                v.element.innerHTML = v.ui.css() + v.ui.exportMenuButton(v.config) + C, v._attachInitEventHandlers(), v._attachEventHandlers(), v.config.showXScroll === OrgChart.scroll.visible && (v.xScrollUI.create(v.width(), v.config.padding), v.xScrollUI.setPosition(), v.xScrollUI.addListener(v.getSvg())), v.config.showYScroll === OrgChart.scroll.visible && (v.yScrollUI.create(v.height(), v.config.padding), v.yScrollUI.setPosition(), v.yScrollUI.addListener(v.getSvg())), v.config.enableSearch && v.searchUI.addSearchControl(), v.toolbarUI.init(v, v.config.toolbar);
                var h = document.createElement("div");
                h.setAttribute("id", "bg-tests"), v.element.appendChild(h)
            }
            var d = !1;
            ...
            if (0 < c[0].length) {
                ...
                for (var g = 0; g < c[0].length; g++) c[0][g] = v.getNodeElement(c[0][g]);
                OrgChart.anim(c[0], c[1], c[2], v.config.anim.duration, v.config.anim.func, function () {
                    d || (y && y(), OrgChart.events.publish("redraw", [v]), v._showAfterAnimation(), d = !0)
                })
            }
            m === OrgChart.action.centerNode ? OrgChart.anim(v.getSvg(), {
                ...
                v.xScrollUI.setPosition(), v.yScrollUI.setPosition(), d || (y && y(), OrgChart.events.publish("redraw", [v]), v._showAfterAnimation(), d = !0)
            }) : !i || !v.response || i[0] == v.response.viewBox[0] && i[1] == v.response.viewBox[1] && i[2] == v.response.viewBox[2] && i[3] == v.response.viewBox[3] || m !== OrgChart.action.insert && m !== OrgChart.action.expandCollapse && m !== OrgChart.action.update && m !== OrgChart.action.init ? 0 == c[0].length && (d || (y && y(), OrgChart.events.publish("redraw", [v]), d = !0)) : OrgChart.anim(v.getSvg(), {
                viewbox: i
            }, {
                viewbox: v.response.viewBox
            }, 500, OrgChart.anim.inOutPow, function () {
                v.xScrollUI.setPosition(), v.yScrollUI.setPosition(), d || (y && y(), OrgChart.events.publish("redraw", [v]), d = !0)
            }), v._initialized = !0
        } else {
            var p = t.boundary,
                f = p.maxX - p.minX,
    ...
    var r = this.get(t.id);
    if (!0 === e && this.config.onUpdate && !1 === OrgChart.events.publish("update", [this, r, t])) return !1;
            ...
            for (var e in p[t]) {
                ...
                switch (e.toLowerCase()) {
                    ...
                    case "margin":
                        ...
                        break;
                    default:
                        o = u((10 * y - 10) / f) * (p[t][e] - g[t][e]) + g[t][e], c[t] && c[t].style && (c[t].style[e] = o + r)
                }
            }
        m && m(), 1 + v < (y += 1) && (clearInterval(O), C && C(c))
    }, 10)
    ...
    this._draw(!1, OrgChart.action.exporting, t, function (t) {
        ...
        if (r.innerHTML = t, "svg" == i.ext)
            ...
            if (!1 === OrgChart.events.publish("exportstart", [n, i, r.querySelector("svg"), t])) return !1;
            ...
            a ? a(n, e, r.querySelector("svg")) : (e = JSON.stringify(e), OrgChart._ajax(n.config.exportUrl + "/v2", "POST", e, "arraybuffer", function (t) {
                if (!1 === OrgChart.events.publish("exportend", [n, i, t])) return !1;
    ...
    var r = e.getAttribute("width"),
        ...
        d = a[2],
        c = a[3] / l;
    e.setAttribute("width", n.w), e.setAttribute("height", n.h), e.setAttribute("preserveAspectRatio", "xMidYMin meet");
    ...
    e.setAttribute("preserveAspectRatio", "xMinYMin slice"), e.setAttribute("width", h.w), e.style.display = "";
            ...
            if (n.call(a, t) && a[t])
                for (var r = 0; r < a[t].length; r++) {
                    ...
                    if (!i.event_id || i.event_id == e[0]._event_id) {
                        if (1 == e.length) return i.listener(e[0]);
                        if (2 == e.length) return i.listener(e[0], e[1]);
                        if (3 == e.length) return i.listener(e[0], e[1], e[2]);
                        if (4 == e.length) return i.listener(e[0], e[1], e[2], e[3]);
                        if (5 == e.length) return i.listener(e[0], e[1], e[2], e[3], e[4])
                    }
                }
        }
    }
    ...
    for (var a = t.nodes, n = {}, o = [], l = [], s = [], h = {}, d = !1, c = 0; c < a.length; c++) {
        ...
        if ((v = new OrgChart.node(g.id, g.pid, C, p, u)).isAssistant = -1 != p.indexOf("assistant"), v.isAssistant && -1 == s.indexOf(v.pid) && s.push(v.pid), null != f) {
            if (!n[f.name]) {
                o.push(f.name);
                var m = new OrgChart.node(f.name, g.pid, null, [f.name], f.tag.template);
                m.isGroup = !0, null != t.orderBy && (m.order = g[t.orderBy]), n[f.name] = m
            }
            v.isChildOfGroup = !0, v._groupParentNodeId = f.name, n[f.name].bgnodes.push(v), h[v.id] = f.name, d = !0
        } else null != t.orderBy && (v.order = g[t.orderBy]), n[g.id] = v, o.push(g.id)
    }
    ...
    for (c = 0; c < o.length; c++) {
        var y = o[c],
            v = n[y],
            x = i ? i[y] : null;
        if (null != v.pid && null != v.pid && "" !== v.pid)(_ = n[v.pid]) ? ((v.parent = _).children.push(v), _.childrenIds.push(v.id)) : (v.pid = null, v.parent = null, l.push(v));
        else l.push(v);
        e == OrgChart.action.exporting && null != r.id && (l = [n[r.id]]), x ? (v.state = x.state, v.state == OrgChart.COLLAPSE && O.push(v.id), OrgChart.server._setNodeGroupState(v, x.groupState, t.orientation)) : OrgChart.server._setNodeGroupState(v, OrgChart.EXPAND, t.orientation), OrgChart.server._initStates(v, n, e, t, r, h, O), OrgChart.server._initGroupStates(v, n, e, t, r)
    }
    if (null != t.roots) {
        l = [];
            ...
            if (b) {
                if (b.parent)
                    for (var w = (_ = b.parent).children.length - 1; 0 <= w; w--) {
                        (j = _.children[w]) == b && (j.parent = null, _.children.splice(w, 1), _.childrenIds.splice(w, 1))
                    }
                l.push(b)
            }
        }
    ...
    for (c = 0; c < s.length; c++) {
        ...
        (x = i[(n[k.id] = k).id]) && (n[k.id].state = x.state, n[k.id].state == OrgChart.COLLAPSE && O.push(k.id)), OrgChart.server._initStates(k, n, e, t, r, h, O);
        var S = [];
        for (w = _.children.length - 1; 0 <= w; w--) {
            (j = _.children[w]).isAssistant ? (j.parent = null, _.children.splice(w, 1), _.childrenIds.splice(w, 1), S.splice(0, 0, j.id)) : (j.pid = k.id, (j.parent = k).children.push(j), k.childrenIds.push(j.id), _.children.splice(w, 1))
        }
        if (S.length % 2) {
            ...
            OrgChart.templates[I] = {
                ...
                nodeMenuButton: "",
                size: A.size
            }, (J = new OrgChart.node(_.id + "_transperant", N.pid, null, [], I)).isAssistant = !0, (x = i[(n[J.id] = J).id]) && (n[J.id].state = x.state, n[J.id].state == OrgChart.COLLAPSE && O.push(J.id)), OrgChart.server._initStates(J, n, e, t, r, h, O), S.splice(S.length - 1, 0, J.id)
        }
        var E = 1;
        for (w = S.length - 1; 0 <= w; w--)
            if (w % 2 && w != S.length - 1) {
                var M = new OrgChart.node(_.id + "_split_" + E, null, null, [], "split");
                (x = i[(n[M.id] = M).id]) && (n[M.id].state = x.state, n[M.id].state == OrgChart.COLLAPSE && O.push(M.id)), OrgChart.server._initStates(M, n, e, t, r, h, O), S.splice(w, 0, M.id), E++
            } else w % 2 && S.splice(w, 0, k.id);
        ...
        for (var U = z.id; null != _;) {
            if (-1 != (G = O.indexOf(_.id)) && O.splice(G, 1), _.state = OrgChart.EXPAND, r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
                for (c = 0; c < _.childrenIds.length; c++) {
                    ...
                    if (!OrgChart.server._isSplit(j) && !j.isAssistant && j.id != U) - 1 == (G = O.indexOf(_.childrenIds[c])) && O.push(_.childrenIds[c]), j.state = OrgChart.COLLAPSE
                }
            ...
            U = _.id, _ = OrgChart.server._getRealParent(_)
        }
        if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
            for (c = 0; c < z.childrenIds.length; c++) {
                if (-1 != (G = O.indexOf(z.childrenIds[c])) && O.splice(G, 1), (j = n[z.childrenIds[c]]).state = OrgChart.EXPAND, !j.isAssistant && !OrgChart.server._isSplit(j))
                    for (w = 0; w < j.childrenIds.length; w++) {
                        var G, R = n[j.childrenIds[w]]; - 1 == (G = O.indexOf(j.childrenIds[w])) && O.push(j.childrenIds[w]), R.state = OrgChart.COLLAPSE
                    }
            }
    }
    ...
    for (c = 0; c < O.length; c++) {
        if (_ = (v = n[O[c]]).parent)
            for (w = _.children.length - 1; 0 <= w; w--) {
                if (_.children[w] == v)
                    if (-1 == _.collapsedChildrenIds.indexOf(v.id) && _.collapsedChildrenIds.push(v.id), _.children.splice(w, 1), "split" == v.templateName || "split" == _.templateName) {
                        if (L = v.parent) {
                            for (;
                ...
                if (_ = v.parent)
                    for (w = _.children.length - 1; 0 <= w; w--) {
                        if ((j = _.children[w]) != v && 0 == j.childrenIds.length && !j.isAssistant && "split" != j.templateName) {
                            j.parent = v, j.pid = v.id, j.layout = OrgChart.mixed, _.children.splice(w, 1);
                            var Y = v.children.length;
                            v.children[Y] = j, v.layout = OrgChart.mixed
                        }
                    }
        }
    if (t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
        var V = [];
        for (c = 0; c < o.length; c++) {
            if (0 == (v = n[o[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)(_ = v.parent) && -1 == V.indexOf(_.id) && V.push(_.id)
        }
        for (c = 0; c < V.length; c++) {
            ...
            for (w = _.children.length - 1; 0 <= w; w--) {
                if (0 == (j = _.children[w]).childrenIds.length && !j.isAssistant && "split" != j.templateName && j.parent && "split" != j.parent.templateName) {
                    if ((j = _.children[w]).parent = null, j.layout = OrgChart.tree, _.children.splice(w, 1), _.childrenIds.splice(w, 1), t.layout == OrgChart.treeRightOffset && W.splice(0, 0, j.id), t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
                        var J;
                        ...
                        OrgChart.templates[I] = {
                            ...
                            nodeMenuButton: "",
                            size: [0, 0]
                        }, (J = new OrgChart.node(j.id + "_transperant", null, null, [], I)).layout = OrgChart.tree, n[J.id] = J, W.splice(0, 0, J.id)
                    }
    ...
    if (C.top = l - i.padding, C.left = o - i.padding, C.bottom = n + i.padding - e / v, C.right = a + i.padding - t / v, C.maxX = a, C.minX = o, C.maxY = n, C.minY = l, null == s && i.align == OrgChart.CENTER) {
        var x = Math.ceil(t / v),
            b = Math.ceil(e / v),
            w = 0,
            _ = 0;
        ...
        x = Math.ceil(t / v), b = Math.ceil(e / v), w = 0, _ = 0;
    ...
    node: function (t, e, r, i, a, n, o, l, s, h) {
        ...
        y = y.replace("{opacity}", v), t.isGroup || null == i.nodeMenu || "split" == t.templateName || l === OrgChart.action.exporting || (c += d.nodeMenuButton.replace("{id}", t.id));
        var x = OrgChart.ui._renderGroupedNodes(t, d, i, r, o, a, n, l, s);
        return t.isGroup && l != OrgChart.action.exporting && (t.groupState == OrgChart.EXPAND ? c += OrgChart.MINIMIZE.replace("{x}", t.w - 45).replace("{id}", t.id) : c += OrgChart.MAXIMIZE.replace("{x}", t.w - 45).replace("{id}", t.id)), c = y + c + x + OrgChart.grCloseTag
    },
                ...
                for (var f = e; null == h;) {
                    ...
                    if (!u) {
                        ...
                        if (C.parent) switch (g = r.levelSeparation, C.parent.layout != OrgChart.mixed && C.parent.layout != OrgChart.tree || (g = r.mixedHierarchyNodesSeparation), r.orientation) {
                            ...
                            case OrgChart.orientation.left:
                            case OrgChart.orientation.left_top:
                                y = C.parent.x + C.parent.w + g * (2 / 3), v = l[l.length - 1][1];
                                break;
                            case OrgChart.orientation.right:
                            case OrgChart.orientation.right_top:
                                y = C.parent.x - g * (2 / 3), v = l[l.length - 1][1]
                        }
                        l.push([y, v])
                    }
                    f = C
    ...
    if ("multiline" == s) {
        ...
        if (null != d.innerHTML) y = d.innerHTML, d.innerHTML = "";
            ...
            var v = "";
            for (m = d.childNodes.length - 1; 0 <= m; m--) v = XMLSerializer().serializeToString(d.childNodes[m]) + v, d.removeChild(d.childNodes[m]);
            y = v
        }
        return y
    }
...
}, OrgChart.pdfPrevUI = {}, OrgChart.loc || (OrgChart.loc = {}), OrgChart.loc.ppdfCmdTitle = "PDF Preview", OrgChart.loc.ppdfSave = "Save", OrgChart.loc.ppdfCancel = "Cancel", OrgChart.loc.ppdfSize = "Size", OrgChart.loc.ppdfFitToDrwaing = "Fit To Drwaing", OrgChart.loc.ppdfLayout = "Layout", OrgChart.loc.ppdfPortrait = "Portrait", OrgChart.loc.ppdfLandscape = "Landscape", OrgChart.loc.ppdfFittopagewidth = "Fit to page width", OrgChart.loc.ppdfMargin = "Margin", OrgChart.loc.ppdfHeader = "Header", OrgChart.loc.ppdfFooter = "Footer", OrgChart.pdfPrevUI.show = function (r, i) {
    ...
    "fit" == t.value ? (e.parentNode.style.display = "none", r.parentNode.style.display = "none") : (e.parentNode.style.display = "block", r.parentNode.style.display = "block")
    ...
    for (var o = n.querySelector("svg"), l = t.element.querySelector("#bg-ppdf-content"), s = 0; s < a.pages.length; s++) {
        ...
        Object.assign(h.style, {
            ...
            "box-shadow": "0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)"
        }), l.appendChild(h), o && o.style.backgroundColor ? h.style.backgroundColor = o.style.backgroundColor : h.style.backgroundColor = "#fff";
        var d = h.contentWindow.document;
        d.open(), "A4" == i.format && i.landscape ? (h.style.width = OrgChart.A4h + "px", h.style.height = OrgChart.A4w + "px") : "A4" == i.format ? (h.style.width = OrgChart.A4w + "px", h.style.height = OrgChart.A4h + "px") : "fit" == i.format && (h.style.width = a.size.w + "px", h.style.height = a.size.h + "px", l.style.marginTop = 0, l.style.marginBottom = 0, h.style.margin = "0 auto"), a.pages[s].html ? (a.pages[s].backgroundColor && (h.style.backgroundColor = a.pages[s].backgroundColor), d.write(OrgChart._exportHtml(a.pages[s].html, i, a, s + 1, a.pages.length))) : (o.setAttribute("viewBox", a.pages[s].vb), d.write(OrgChart._exportHtml(n.innerHTML, i, a, s + 1, a.pages.length)));
        var c = d.getElementById("bg-header"),
            g = d.getElementById("bg-footer");
        c.style.top = i.margin[0] - c.offsetHeight - 7 + "px", g.style.bottom = i.margin[2] - g.offsetHeight - 7 + "px", d.close()
    }
    ...
    for (var n = "", o = 0; o < e.margin.length; o++) n += e.margin[o] + "px ";
